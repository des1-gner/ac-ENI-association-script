...
Please run the following validation script to check for VPC configuration conflicts:
...
if [ "$OVERLAP_FOUND" = true ]; then
    echo ""
    echo "❌ CANNOT PROCEED: Found agents using the same subnet and security group combination"
    echo "ENIs are shared across agents with matching VPC configurations"
    echo "SOLUTION: Delete the conflicting agents first. ENIs will be automatically cleaned up 8-10 hours after agent deletion."
    exit 1
else
    echo ""
    echo "✅ No VPC configuration overlaps found - safe to proceed with cleanup"
    exit 0
fi
Step 2: Run the script

chmod +x validate_eni_cleanup.sh
./validate_eni_cleanup.sh <eni-id> <region>
